...
beta = 1 ## preassigned value of beta ##
...
e2.boot<- c()  ## second difference of induced centered errors ##
for(i in 1:n)
{
  if(i==1)
  {
    e2.boot[i]<- e.sort[2]-e.sort[1]
  }
  else if (i==n)
  {
    e2.boot[i]<- e.sort[n-1]-e.sort[n]
  }
  else
  {
    e2.boot[i]<- e.sort[i+1]-2*e.sort[i]+e.sort[i-1]
  }
}

emp.cdf.e2.H0 = ecdf(e2.boot)(e2.boot) ## empirical cdf of second difference of induced centered errors ##
...
for(i in 1:n)
{
  if(i==1)
  {
    y2.boot[i]<- y.sort[2]-y.sort[1]
  }
  else if (i==n)
  {
    y2.boot[i]<- y.sort[n-1]-y.sort[n]
  }
  else
  {
    y2.boot[i]<- y.sort[i+1]-2*y.sort[i]+y.sort[i-1]
  }
}

data.xy<- cbind(x.sort,y2.boot)  ## data on ordered X-observations and second difference of induced resampled responses ##
...
points.all<- cbind(data.pts.x1, data.pts.y1) ## bivariate quantiles on X and induced second order difference of Y ##

u = ecdf(dt.x)(x.sort) ## empirical CDF values of X ##
v = ecdf(dt.y)(y2.boot) ## empirical CDF values of induced Y ##
...
## conditional error observations of size n under H1 ##
k1<- c()
k2<- c()
k3<- c()
e.x<- c()
for(i in 1:n)
{
  k1[i]<- (0.2*data.xy[,1][i])^(1/4)
  k2[i]<- 10*sqrt(k1[i])
  k3[i]<- sqrt(1-k1[i])
  e.x[i]<- k3[i]*rt(1, round(2/k1)[i], 0)
}
...
for(i in 1:n)
{
  ...
  C[i]<- (k2[i]*y1[i]*f1[i]*w2[i]^-f2[i])/(k3[i]*w1[i])
  h1[i]<- C[i]-1
}
...
aij = function(i,j) {mean(h1*2*sin(pi*i*x.sort)*sin(pi*j*y2.boot))} ## calculation of the coefficients a_ij's ##
...
e2.boot1<- c()  ## second order difference of conditional error under H1 ##
for(i in 1:n)
{
  if(i==1)
  {
    e2.boot1[i]<- e.x[2]-e.x[1]
  }
  else if (i==n)
  {
    e2.boot1[i]<- e.x[n-1]-e.x[n]
  }
  else
  {
    e2.boot1[i]<- e.x[i+1]-2*e.x[i]+e.x[i-1]
  }
}

dataset1 = cbind(x.sort,e2.boot1)  ## dataset on ordered X-observations and second order difference of conditional error under H1 ##

emp.cdf.e2.H1 = ecdf(e2.boot1)(e2.boot1) ## empirical CDF of second order difference of conditional error under H1 ##

## Calculation of mean of asymptotic distribution of Tn1 under contiguous alternatives Hn ##
U = 2*emp.cdf.x*emp.cdf.e2.H0+2*(1-emp.cdf.x)*(1-emp.cdf.e2.H0)-1
V = 2*emp.cdf.x*emp.cdf.e2.H1+2*(1-emp.cdf.x)*(1-emp.cdf.e2.H1)-1
...
L<- matrix(0,n1,n1)  ## Matrix with elements being the values of kernel function of Tn2 approximated at the bivariate marginal quantiles of X and second order difference of Y ##
{
  for(p in 1:n1)
  {
    for(q in 1:n1)
...
lmat2<- (L+t(L))/2  ## Symmeytrization of L as lmat2, since L and lmat2 have same eigenvalues and eigenvectors. ##
...
plot(gm, KS, xlim=c(0,10), ylim=c(0,1), type="l", pch=10, col="red", xlab=expression(gamma), ylab=expression("Semiparametric Powers"))
# Add a line
